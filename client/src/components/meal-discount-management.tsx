...
export function MealDiscountManagement() {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // Fetch global meal discounts
  const { data: globalDiscounts, isLoading: isLoadingGlobal } = useQuery({
    queryKey: ["/api/meal-discounts/global"],
    queryFn: async () => {
      const res = await fetch("/api/meal-discounts/global");
      if (!res.ok) throw new Error("Failed to fetch global meal discounts");
      return res.json();
    },
  });
  ...
  const updateGlobalMutation = useMutation({
    mutationFn: async (data: { zombieDiscount: number; survivantDiscount: number; staffDiscount: number }) => {
      ...
      if (!res.ok) throw new Error("Failed to update global meal discounts");
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/meal-discounts/global"] });
      toast({
        title: "Réductions repas mises à jour",
        description: "Les réductions par type pour les repas ont été mises à jour avec succès",
      });
    },
    onError: () => {
      toast({
        title: "Erreur",
        description: "Impossible de mettre à jour les réductions repas",
        variant: "destructive",
      });
    },
  });
  ...
  const updateSquadMutation = useMutation({
    mutationFn: async ({ squadId, discount }: { squadId: number; discount: number }) => {
      const res = await fetch(`/api/meal-discounts/squad/${squadId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ discount }),
      });
      if (!res.ok) throw new Error("Failed to update squad meal discount");
      return res.json();
    },
    onSuccess: () => {
      toast({
        title: "Réduction squad repas mise à jour",
        description: "La réduction repas pour ce squad a été mise à jour",
      });
    },
    onError: () => {
      toast({
        title: "Erreur",
        description: "Impossible de mettre à jour la réduction repas du squad",
        variant: "destructive",
      });
    },
  });
  ...
  const updateParticipantMutation = useMutation({
    mutationFn: async ({ participantId, discount }: { participantId: number; discount: number | null }) => {
      const res = await fetch(`/api/meal-discounts/participant/${participantId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ discount }),
      });
      if (!res.ok) throw new Error("Failed to update participant meal discount");
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/meal-discounts/participant"] });
      toast({
        title: "Réduction participant repas mise à jour",
        description: "La réduction individuelle repas a été mise à jour",
      });
    },
    onError: () => {
      toast({
        title: "Erreur",
        description: "Impossible de mettre à jour la réduction repas du participant",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <Tabs defaultValue="global" className="space-y-6">
      ...
      <TabsContent value="global">
        <Card>
          <CardHeader>
            <CardTitle>Réductions Repas par Type de Joueur</CardTitle>
            <CardDescription>
              Définissez des réductions en pourcentage pour les repas selon le type de joueur
            </CardDescription>
          </CardHeader>
      ...
      <TabsContent value="squad">
        <Card>
          <CardHeader>
            <CardTitle>Réductions Repas par Squad</CardTitle>
            <CardDescription>
              Définissez des réductions repas spécifiques pour chaque squad
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <TabsContent value="participant">
        <Card>
          <CardHeader>
            <CardTitle>Réductions Repas par Participant</CardTitle>
            <CardDescription>
              Définissez des réductions individuelles pour les repas (priorité absolue)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
...
function SquadDiscountRow({
  ...
  const { data: squadDiscount } = useQuery({
    queryKey: [`/api/meal-discounts/squad/${squad.id}`],
    queryFn: async () => {
      const res = await fetch(`/api/meal-discounts/squad/${squad.id}`);
      if (!res.ok) throw new Error("Failed to fetch squad meal discount");
...
function ParticipantDiscountRow({
  ...
  const { data: participantDiscount } = useQuery({
    queryKey: [`/api/meal-discounts/participant/${participant.id}`],
    queryFn: async () => {
      const res = await fetch(`/api/meal-discounts/participant/${participant.id}`);
      if (!res.ok) throw new Error("Failed to fetch participant meal discount");
      const data = await res.json();
