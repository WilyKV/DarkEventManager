...
export default function RepasPage() {
  ...
  const handleScan = async (qrData: string) => {
    try {
      ...
      // Get meal discount for participant
      const discountRes = await fetch(`/api/meal-discounts/calculate/${participant.id}`);
      if (discountRes.ok) {
  ...
  const createPurchasesMutation = useMutation({
    mutationFn: async () => {
      ...
      const promises = purchases.map(async (purchase) => {
        ...
        if (!purchaseRes.ok) throw new Error("Failed to create meal purchase");

        // Update stock (decrement)
        const item = cart.find(i => i.id === purchase.mealItemId);
        if (item) {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "L'achat de repas a été enregistré avec succès",
      });
    ...
    onError: () => {
      toast({
        title: "Erreur",
        description: "Impossible d'enregistrer l'achat de repas",
        variant: "destructive",
      });
    },
  });
...
function ProductCard({
  ...
  const IconComponent = item.icon ? ICON_MAP[item.icon] || Utensils : Utensils;
  const originalPrice = parseFloat(item.price);
  const discountedPrice = originalPrice * (1 - discount / 100);
